...
class TaskList {
    ...
    getMarkTask(id) {
        ...
        document.getElementById("taskTitle-" + id).classList.toggle("titleDone");
        document.getElementById("verifyed-" + id).classList.toggle("taskDone");
    }

    getEditTask(id) {
        const oldTask = document.getElementById('taskTitle-' + id).innerHTML;
...
function deleteTask(id) {
    taskList.getDeleteTask(id);
    showHtml();
}

function doneTask(id) {
    taskList.getMarkTask(id);
}

let editing = false;

function editTask(id) {
    taskList.getEditTask(id);
    editing = id;
}


function createTasks() {
    const inpTask = document.getElementById('task').value;
    if (editing !== false) {
        taskList.getNewTaskEdited(inpTask);
        showHtml();
        cleanFields();
        editing = false;
    } else {
        const taskId = generateId();
        const task = new Task(inpTask, taskId, false);
        taskList.getNewTask(task);
        showHtml();
        cleanFields();
    }
}
